...
void controlMenu(){
   boolean back=false;
  do{
    ecoderMovimiento(auxPosMenu,1);
    if(auxPosMenu==0||auxPosMenu==1){//se muestre cada opcion en la pantalla
      lcd.setCursor(1,0);
      lcd.print("Menu config");
      lcd.setCursor(1,1);
      lcd.print("Sistema ");
    }
    seleccionado(auxPosMenu);
    if(ecoderPresionado()){
      back=pickMainMenu(auxPosMenu);
    }
  }while(back==false);
  auxPosMenu=0;
  lcd.clear();
  }
boolean pickMainMenu(int prmPos){
  if(prmPos==0){
    configuracion();
  }
  if(prmPos==1){
  ...
  do{
    ...
    seleccionado(posSetMenu);
    if(ecoderPresionado()){
      back=opcionesMainMenu();
    }
  }while(back==false);
  posSetMenu=0;
  lcd.clear();
...
void mainTemp(){
  ...
  do{
    ...
    seleccionado(auxPosMenu);
    if(ecoderPresionado()){
      back=opcionesTemp(auxPosMenu);
    }
  }while(back==false);
  auxPosMenu=0;
  lcd.clear();
}
boolean opcionesTemp(int pos){
  if(pos==0){
    opcionConfTempH();
  }
  if(pos==1){
...
void mainLuz(){
  ...
  do{
    ...
    seleccionado(auxPosMenu);
    if(ecoderPresionado()){
      back=opcionesLuz(auxPosMenu);
    }
  }while(back==false);
  auxPosMenu==0;
  lcd.clear();
}
boolean opcionesLuz(int pos){
  if(pos==0){
    opcionConfLuz();
  }
  if(pos==1){
...
void mainMicro(){
  ...
  do{
    ...
    seleccionado(auxPosMenu);
    if(ecoderPresionado()){
      back=opcionesMicro(auxPosMenu);
    }
  }while(back==false);
  auxPosMenu==0;
  lcd.clear();
}
boolean opcionesMicro(int pos){
  if(pos==0){
    opcionConMicro();
  }
  if(pos==1){
...
void opcionConfTempL(){
 ...
 void opcionConfLuz(){
    ...
    if (key){
      //guarda cada caracter ingresado en el arreglo y muestra un * por cada caracter
      lcd.setCursor(pos3++,1);
      lcd.print(key);
      caden3=caden3+key;
      temConf3= caden3.toInt();
